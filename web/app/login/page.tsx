...
export default function LoginPage() {
  ...
  const handleZkLogin = async () => {
    ...
    setError("");
    try {
      // 1. Initialize session: generates ephemeral key, randomness, nonce → saved to localStorage
      const { nonce } = await ZkLoginService.initializeSession();

      // 2. Build Google OAuth URL with the nonce embedded
  ...
  return (
    <div className="min-h-screen bg-[#fafbfc] flex items-center justify-center p-6">
      <div className="w-full max-w-md space-y-8">
        ...
        {/* Login Card */}
        <div className="bg-white rounded-[40px] shadow-2xl shadow-black/5 border border-slate-100 p-10 space-y-8">
          <div className="space-y-2">
            <h2 className="text-2xl font-black text-[#1a1a1a] tracking-tight">
              Sign in
            </h2>
            <p className="text-sm text-slate-500 leading-relaxed">
              Use your Google account to generate a self-custodial Sui wallet via zero-knowledge proof. No seed phrase required.
            </p>
          </div>
        ...
        <p className="text-center text-[11px] text-slate-400">
          Powered by{" "}
          <span className="font-bold text-slate-600">Sui zkLogin</span>
